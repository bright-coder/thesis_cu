          ...
          <form v-on:submit.prevent="addChangeRequest">
            <div class="modal-body">
              <div v-if="changeRequest.changeType == 'add' || changeRequest.changeType == 'edit'">
                <div class="div" v-if="changeRequest.changeType == 'add' && addType == 0">
                  <button type="button" class="btn btn-success" @click="addType = 1"> add new input</button>
                  <hr>
                  <label class="col-form-label"> add input from project</label><br>
                  <select v-model="changeRequest.name" class="form-control" required @change="setColTable(changeRequest.name)">
                    <option v-for="(input, indexInputAddList) in listInputAdd" :key="indexInputAddList" v-bind:value="input.name">{{ input.name }}</option>
                  </select>
...
export default {
  ...
  methods: {
    ...
    setColTable(name){
      for(let i = 0; i < this.listInputAdd.length; ++i) {
    ...
    getInputList() {
      ...
      for (let i = 0; i < this.functionalList.length; ++i) {
        for (let j = 0; j < this.functionalList[i].inputs.length; ++j) {
      ...
      for (
        let i = 0;
        i < this.functionalList[this.selectedFunctional].inputs.length;
        ++i
      ) {
        delete objIndex[
          this.functionalList[this.selectedFunctional].inputs[i].name
        ];
      }
    ...
    isInFr(name) {
      for (
        ...
        if (
          this.functionalList[this.selectedFunctional].inputs[i].name == name
        ) {
          return true;
        }
      }
    ...
    validateSameColumnAndTable(table, column) {
      ...
      for (let i = 0; i < this.functionalList.length; ++i) {
        for (let j = 0; j < this.functionalList[i].inputs.length; ++j) {
          if (
            table == this.functionalList[i].inputs[j].tableName &&
            column == this.functionalList[i].inputs[j].columnName
          ) {
            this.errors = "this column already is use.";
            return false;
          }
    ...
    validateAdd(crInput) {
      ...
      if (
        !this.validateSameColumnAndTable(crInput.tableName, crInput.columnName)
      ) {
        return false;
      }
    ...
    validateEdit(crInput) {
      ...
      let refDatype =
        "dataType" in crInput ? crInput.dataType : oldInput.dataType;
